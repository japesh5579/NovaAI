...
const testimonials = [
  ...
  },
  {
    ...
    stars: 5,
    accent: "from-pink-500/15 to-violet-500/5",
    dot: "bg-pink-400",
    initials: "TB",
  },
];
...
export default function TestimonialsMarquee() {
  return (
    <section className="relative w-full overflow-hidden bg-[#07070f] py-20 border-b border-white/[0.05]">

      <svg
        className="absolute right-0 bottom-0 text-blue-500/5 pointer-events-none"
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mx-auto mb-14 flex max-w-3xl flex-col items-center px-6 text-center">
          <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-cyan-400 shadow-lg shadow-blue-500/20">
        ...
        <div className="relative w-full">
          <div className="pointer-events-none absolute top-0 left-0 z-10 h-full w-32 bg-gradient-to-r from-[#07070f] to-transparent" />
          <div className="pointer-events-none absolute top-0 right-0 z-10 h-full w-32 bg-gradient-to-l from-[#07070f] to-transparent" />
          ...
          <Marquee className="[--gap:1.25rem]" pauseOnHover speed={45}>
            {testimonials.map((t) => (
              <TiltCard
                ...
                className={`w-80 shrink-0 rounded-2xl border border-white/[0.07] bg-gradient-to-br ${t.accent} bg-[#0a0a18] p-6 flex flex-col gap-4 hover:border-white/[0.14] transition-colors duration-300`}
              >
                {/* Stars */}
                ...
                {/* Quote */}
                <p className="text-[12px] text-gray-400 leading-relaxed flex-1">&ldquo;{t.quote}&rdquo;</p>

                {/* Author */}
                <div className="flex items-center gap-3 pt-1 border-t border-white/[0.05]">
                  <div className={`w-8 h-8 rounded-full ${t.dot} bg-opacity-20 border border-white/10 flex items-center justify-center shrink-0`}
                    style={{ background: "rgba(255,255,255,0.06)" }}>
                    <span className="text-[9px] font-black text-white/70">{t.initials}</span>
                  </div>
                  <div>
                    <p className="text-[12px] font-bold text-white leading-none">{t.name}</p>
                    <p className="text-[10px] text-gray-600 mt-0.5">{t.role}</p>
                  </div>
